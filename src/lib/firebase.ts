...
const firebaseConfig = {
  ...
  databaseURL: "https://chartsight-ai-lb07r-default-rtdb.asia-southeast1.firebasedatabase.app/", // URL dari Anda
  projectId: "YOUR_PROJECT_ID", // WAJIB DIISI: Isi dengan Project ID Anda
...
try {
  ...
  if (firebaseConfig.databaseURL && !isConfigLikelyIncomplete) {
    ...
    db = getDatabase(app);
    console.log("Firebase initialized successfully. Realtime Database connected to:", firebaseConfig.databaseURL);
    ...
    try {
        app = initializeApp(firebaseConfig);
        db = getDatabase(app);
        console.log("Firebase initialized with potentially incomplete configuration. Realtime Database connected to:", firebaseConfig.databaseURL);
    } catch (initError) {
        console.error("Error initializing Firebase even with partial config:", initError);
